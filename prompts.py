...
You are a highly skilled assistant with expertise in working with Contentstack schemas and converting RTE-formatted webpage outputs into a valid Contentstack entry. I will provide you with:

An RTE-formatted JSON structure that represents a webpage.
A Contentstack Entry schema that details how fields map to various data types (text, date, file, RTE, reference, global fields, etc.).
Your Task:

Read the provided Contentstack Entry schema carefully.
Review the given RTE-formatted webpage output.
Convert the RTE output into a single Contentstack entry JSON that aligns with the schema. This means you must:
Identify which parts of the RTE correspond to the schema's fields (e.g., title -> title field, featured image -> featured_image field, body sections -> body RTE field, related posts links -> related_post references, comments and call to action -> comments group, social icons -> social_share, SEO meta fields -> seo global field, etc.).
Populate every field defined in the schema using content extracted from the RTE output. For fields like date, use a valid ISO-8601 date. For file fields (e.g., featured_image), use a plausible placeholder image URL extracted from the RTE. For reference fields, extract the related post links and represent them as reference UIDs or placeholder UIDs. For SEO fields, map the meta tags you represented in RTE back into their respective text or boolean fields.
For the RTE field in the schema (like body), ensure that the final field is in Contentstack's JSON RTE format. If you already have it in valid RTE format from the input, incorporate it directly. If it needs adjustments, do so while preserving the formatting requirements (e.g., headings, bold, italic, lists, etc.).
Output Requirements:

Produce a single JSON object that is a valid Contentstack entry matching the provided schema.
Do not include extra explanations.
Only output the final Contentstack entry JSON object.

An example of Contentstack Entry that you have to take is given below ( for reference is given below ): 
{entry_contentstack_example}

Given below is the input and make sure to fill each key with some value if the data is not present. 
In other words, I want to generate the json as big as possible.
{res1}
Finally, IMPORTANT NOTE, MAKE SURE TO RETURN ONLY THE JSON AND NOTHING OTHER THAN THAT
'''
